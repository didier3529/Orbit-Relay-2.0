...
function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col md:items-center md:justify-center bg-deep-space overflow-hidden relative">
      ...
      {isMobile && (
        <div className="sticky top-0 w-full p-4 flex justify-between items-center z-20 bg-deep-space bg-opacity-90 backdrop-blur-sm border-b border-gray-800">
          <div className="text-white font-bold text-xl">Airship</div>
          <button 
      ...
      {isMobile && mobileMenuOpen && (
        <div className="fixed inset-0 bg-deep-space bg-opacity-95 z-10 p-6 pt-20 flex flex-col space-y-4">
      ...
      <div 
        ref={contentRef}
        className="w-full max-w-4xl z-5 p-4 md:p-0 flex-1 overflow-auto"
      >
        <Routes>
      ...
      {/* Side Dropdowns - only visible on desktop */}
      {!isMobile && (
        <>
